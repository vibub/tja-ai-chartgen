...
Optional stem-role rhythm enhancement:
...
By default, the preparation command now installs only the recommended Demucs `htdemucs` stem-role model under `models/stem-role-v1/`. Run `tja-ai-chartgen prepare-instrument-models --profile full` only when legacy concrete-instrument taxonomy diagnostics are required; that compatibility profile stores Demucs and pinned AST AudioSet weights under `models/instrument-v1/`. Existing users who prepared only `models/instrument-v1/` can keep using explicit `--instrument-profile full`, or run the default preparation command once to move to the recommended stem-role path. `models/` is ignored by Git. Generation never downloads models implicitly and never writes separated stems into the output directory.
...
Optional rhythm enhancement now defaults to `stem-role-v1`. Local Demucs `htdemucs` separates vocals, drums, bass, and other, and maps coarse source activity plus sparse vocal/drum/bass/accompaniment onsets onto canonical bars and ticks. Unified salience, structure, fallback, and AI consumers use these only as bounded rhythm, phrase, and motif evidence. Vocals are not mapped syllable by syllable, and no source evidence can override silence, difficulty, NPS, occupancy, resolution, or playability constraints.

The legacy `full` profile remains compatible with `instrument-v1`: it adds the pinned AST AudioSet classifier and preserves guitar, piano/keyboard, strings, brass, woodwind, synth, and organ fields. Those concrete taxonomy values are diagnostic-only and no longer decide note placement, the current AI payload, or the QualityReport mainline. New jobs run AST only when `--instrument-profile full` is explicitly selected.

Enable the recommended stem-role enhancement:
...
BeatNet is installed with the project's default dependencies. On supported Windows, Linux x86-64, and macOS ARM64 environments, the project also installs `madmom-prebuilt`, which provides Python 3.11–3.13 wheels, and bridges its `madmom` module/distribution-name mismatch. The project supplies the legacy NumPy aliases removed in NumPy 1.24+ and ignores PyAudio when non-streaming analysis does not need it. If dependencies are unavailable, inference fails, or output is invalid, the CLI keeps the default librosa result and records a stable failure category in the notice detail. For an existing development environment, rerun `pip install -e ".[dev]"` to install the new dependencies.
...
Each AI provider transport attempt times out after 300 seconds by default. Use `--ai-request-timeout` to select 1–600 seconds. Timeouts, connection failures, HTTP errors, authentication errors, request-parameter errors, and other provider call exceptions share the same transport retry count and are retried up to 3 times by default; use `--ai-transport-retries 0|1|2|3|4|5` to change this. LiteLLM internal retries are disabled so the project can record the elapsed time and exception category for every transport attempt. Transport retries are independent from content repair: the AI receives the `tja-ai-chartgen-compact-v2` payload and returns sparse `hits` / `long_notes` events on canonical ticks instead of choosing a notes-string length. The shared event encoder validates tick bounds, target-resolution representability, conflicts, and long-note structure. Legacy `notes` output, invalid JSON/bar counts, and quality-gate failures enter content repair, which runs up to 2 times by default and is configurable with `--ai-repair-retries`. Exhausting either boundary falls back to the rule-based generator.
...
The four rule-based course loads are initially calibrated against anonymous aggregate statistics from real four-course charts and are regression-tested through project-generated 120 BPM sparse, 180 BPM dense, adaptive 16/24/48-resolution, and stable→build-up→peak→drop structure WAV fixtures; the same pipeline also verifies `spectral-v1` envelope alignment, non-empty flux, bar/sparse-grid mapping, and graceful fallback. In addition to whole-chart average and peak-bar notes/sec, `quality_report*.json` records active-bar average notes/sec, longest note-stream count/duration, accent coverage, normalized pattern repetition, structure-density correlation, peak contrast, build-up slope agreement, cadence variation, fill-candidate precision, returning-section motif consistency, drum-onset coverage, bass/downbeat alignment, vocal-phrase response, instrument-transition response, confident Stage C coverage, instrument-supported fills, and resolution changes/quantization error. Ordinary hit count, NPS, streams, accents, and repetition are measured by real time or normalized bar position, so equivalent rhythms do not change when encoded at 16, 24, or 48 grids. The quality mainline is now fixed to seven rhythm-alignment areas: note/onset, strong onset, unsupported notes, downbeats, fill bursts, rhythmic quantization, and silent ranges. Only three fixture-calibrated extreme failures enter AI repair. Ordinary load counts only notes `1`–`4`; special-note load remains separate. Structure, instrument, and resolution metrics remain compatibility diagnostics and do not participate in a unified weighted score.
...
- `--use-instrument-analysis` requires optional dependencies and prepared local weights. The default `stem-role` profile runs Demucs only and still adds analysis time and memory use. AST concrete-instrument classification runs only with explicit `--instrument-profile full`; its taxonomy is compatibility diagnostics and still requires human review.
