...
def build_bar_features(raw: AudioAnalysisRaw, max_bars: int | None = None) -> list[BarFeature]:
    ...
    grid_length = bar_length / GRIDS_PER_BAR
    ...
    for index, onset_grids in enumerate(onset_grids_by_bar):
        ...
        accent_16 = [grid for grid in onset_16 if grid in {0, 4, 8, 12}]
        if not accent_16 and onset_16:
            accent_16 = [grid for grid in onset_16 if grid in {0, 8}]
        ...
        bars.append(
            BarFeature(
                ...
                energy=min(1.0, len(onset_16) / GRIDS_PER_BAR),
                onset_16=onset_16,
                accent_16=accent_16,
            )
        )
