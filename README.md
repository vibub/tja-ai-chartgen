...
- Python 3.11+
- ffmpeg
- 可选阶段 C：支持当前 Python/平台的 PyTorch、Demucs 与 Transformers
...
每次执行 `generate` 都会在 TJA 输出旁写入 `generation_config.json`。该文件记录输入路径、元数据、难度、是否生成全难度、风格、密度、`--max-bars`、BPM/OFFSET 覆盖值、拍号覆盖值、BeatNet 开关、阶段 C 开关与设备、可选模型目录、特殊音符开关、AI 开关、最终解析使用的模型名、AI 内容修复次数、请求超时和 transport 重试次数。后续可以使用 `generate-from-config` 用同一组参数重新生成谱面。`ai_attempts*.json` 会分别记录内容校验尝试和实际 transport 尝试，并在最终失败时写入稳定的 fallback 原因。API key 和 base URL 不会写入 `generation_config.json`；如需复跑 AI 生成，请继续通过 `.env`、环境变量或命令参数提供连接配置。
...
- 使用更多真实歌曲和人工游玩结果校准 `spectral-v1`、`instrument-v1` 与结构阈值，并评估哪些频谱、声部、乐器和结构指标适合进入 AI repair 或 CI 门槛。
