...
`generate` 当前数据流：输入音频 → `output/<stem>.ogg` → `analysis.json` → AI 或 fallback `ChartBar` → `song.tja` → `report.txt`。`--bpm` / `--offset` 会在音频分析后覆盖 raw analysis，并影响小节网格、`analysis.json` 和最终 `.tja`；`--max-bars N` 会在小节特征阶段只保留前 N 小节，便于用真实音频快速验证。
