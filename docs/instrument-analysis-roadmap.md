...
| 小目标 | 状态 | 完成详情 | 验证记录 |
| --- | --- | --- | --- |
| 9.1 重音候选升级 | 已完成 | accent salience 统一融合 downbeat、强 onset 局部峰值和 section/phrase/energy/transition 结构变化；downbeat 与强 onset 重合时显式加权。fallback 的 performance 风格只从统一候选中选择大音符，并按 course 限制每小节数量；大音符与前后任意普通击打必须保持超过 0.25 秒的跨小节真实时间间隔，快速 `102` / `1002` 左右手交替型不会升级为大音符；QualityReport 的 accent coverage 同步改用统一候选，并按拍号限制每小节只评估最高优先级候选。 | `pytest tests/test_rhythmic_salience.py tests/test_salience_candidates.py tests/test_fallback_generator.py tests/test_chart_quality.py -q` 覆盖强弱 onset、downbeat 协同、结构起点、Easy 上限、peak 上限与候选覆盖。 |
...
- peak 不通过堆积大音符表达；
- Easy/Normal 限制重音数量；
- 大音符与前后任意击打必须保持超过 0.25 秒真实时间间隔，快速 `102` / `1002` 等左右手交替型只使用普通音符。
...
当前实现将候选保存在 `AudioAnalysisRaw.tempo_candidates`，并通过 `SongAnalysis.tempo_candidates` 写入当前 `analysis_schema_version=11` 的 `analysis.json`。`accepted` 只表示候选通过自身现有合法性或 onset-grid 门控，不代表已经完成候选间仲裁；最终择优和部分采用仍由 10.3、10.4 完成。旧分析缺少该字段时按空列表兼容读取。
...
`TempoAnalysisDecision` 新增 `tempo_source`、`meter_source` 和 `partial_adoption`，部分采用时保留完整候选的 `selected_source`，并记录 `accepted=true`、`reason=adopted-beatnet-meter-downbeat`；最终 analyzer 使用 `<tempo-source>+beatnet-meter`，CLI/Web notice 以 `beatnet-meter-adopted` 明确提示混合来源。`analysis_schema_version=11` 持久化完整诊断；旧分析缺少新字段时分别按 `None`、`None` 和 `False` 兼容读取。手动 BPM/OFFSET/拍号覆盖仍在自动仲裁之后执行，覆盖前的混合来源诊断继续保留。

## 10.4 部分采用

部分采用仅混合基础 BPM 与可靠 BeatNet meter/downbeat，不引入 tempo map 或非均匀 beat grid；具体门控、来源字段与持久化语义见上文 `tempo-arbitration-v2` 说明。

## 10.5 固定 BPM 拟合与变速诊断

`tempo-arbitration-v3` 新增 report-only `tempo-variation-v1`。每个候选的 `TempoMeterEvidence` 记录固定 BPM 理论 interval、逐 beat 累积拟合的平均/P95/最大绝对误差、平均误差相对理论 interval 的比例、偏离理论 interval 超过 8% 的比例、首尾三分之一 interval 中位数漂移，以及最大相邻 interval 跳变。6/8 原始 BeatNet 候选按八分音符 pulse interval 拟合，但最终 BPM 继续使用项目统一的四分音符语义。

歌曲级诊断优先使用具备至少 8 个 interval、时间覆盖率不低于 50% 的原始 BeatNet tracker；否则使用 librosa 原始 tracker，避免用规则重建的 onset-grid 制造零误差。`TempoVariationDiagnostic` 持久化诊断来源、证据数量、全部拟合指标、分类、置信度和 `fixed_bpm_constrained`：

- 固定 BPM 误差低且 interval 稳定时分类为 `stable`；
- 存在显著相邻 interval 跳变时分类为 `possible-tempo-change`；
- 首尾 interval 持续漂移且无突变时分类为 `possible-rubato`；
- interval 高变异且异常比例较高时分类为 `possible-live-performance`；
- tracker interval 少于 8 个或覆盖率不足时分类为 `insufficient-evidence`。

检测到疑似变化时只记录 `fixed_bpm_constrained=true` 并产生 `tempo-variation-suspected` warning，当前仍采用仲裁选出的固定 BPM 和规则 beat grid，不自动生成 `#BPMCHANGE`。只有真实需求明确后，才独立设计 tempo map 与非均匀小节时间轴。`analysis_schema_version=11` 持久化候选指标和歌曲级诊断；旧分析缺少字段时按 `None` 兼容读取。
...
| 13.3 AI payload 移除具体乐器依赖 | 已完成 | `compact-v7` 持久化版本化紧凑 salience、可表达候选与可靠 burst；`bar_instruments` 只保留粗粒度 stem-role，dominant source/confidence 从四类 activity 重新派生，不读取具体乐器 taxonomy 或旧混合 confidence。每小节最多发送 16 个 salience point；低置信最高 resolution 禁止发送任意 finest-grid 微时差，高置信场景也只允许 1 个高置信 onset 例外。 | `tests/test_ai_client.py`、Phase 5 payload taxonomy 独立性验收及旧 compact-v4 sidecar 兼容测试。 |
...
普通 hit 优先消费 reliable strong transient、transient、rhythmic skeleton 和 structure highlight；候选不足时，sustained activity、beat/downbeat、spectral 与受限 stem evidence 只在 course 相关弱补点预算内连接可靠节奏，无证据 style 骨架使用更小预算，sparse/breakdown 不允许无证据补点。performance 大音符只来自统一 accent 排序，并遵守 course 上限及与前后任意击打超过 0.25 秒的跨小节真实时间隔离；快速 `102` / `1002` 等左右手交替型保持普通音符；最终咚咔以 style 为基础票，只有 don/ka preference 超过明确差距才覆盖，不再根据跨小节连续同色次数强制修改 note。
...
后续节奏收敛在不恢复具体乐器 taxonomy 的前提下新增 `rhythm-skeleton-v4`：规则生成器先按统一 salience、ResolutionPlan、course/level/style/density 得到 raw skeleton，再按完整 phrase 选择带相位的直拍、三连音/24 分或保守 mixed lattice；lattice 内 tick 原样保留，lattice 外 tick 不做邻近投影，只有高置信强瞬态在同一精确位置重复出现时才作为受 10% 预算限制的例外，其余孤立微时差移除，并按 density hint 恢复最低 hit 数。compact-v7 payload 将推断后的 musical skeleton 交给 AI，长谱及最高 canonical resolution 的局部重生成会在校验前把 AI 普通 hit 确定性对齐到骨架；稳定格点校验只阻止骨架以外的任意 finest-grid hit，AI 主要负责咚咔顺序、同位置重音与受控 motif 变化。
